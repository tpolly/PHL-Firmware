...
//const uint8_t actor[NUM_TLC59711] = {0, 12}; // actor channels
...
void hvxCallBack(const GattHVXCallbackParams *params) {
  ...
  uint8_t *arr = (uint8_t *)params->data;
  pwm = 1;

  while (*arr) { // null terminated
    Serial.print("Set ");
    uint16_t a = pwm * 0xFFFF;
    Serial.println(a);
    tlc.setPWM(0, a);
    tlc.setPWM(3, a);
    tlc.setPWM(12, a);
...
void tickerTask() {
  tlc.setPWM(4, 0x0FFF * tickerLed);
  tlc.setPWM(16, 0x0FFF * tickerLed);
  tlc.write();
  tickerLed = !tickerLed;
}
