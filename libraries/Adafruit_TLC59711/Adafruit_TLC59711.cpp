...
void Adafruit_TLC59711::write(void) {
  ...
  //noInterrupts(); DISABLED FOR USE WITH BLE NANO, will block and halt CPU on BLE event. This seems to run fine with interrupts enabled.
  ...
  if (_clk >= 0)
    ...
    delayMicroseconds(2);
  //interrupts(); DISABLED FOR USE WITH BLE NANO (see above)
}
